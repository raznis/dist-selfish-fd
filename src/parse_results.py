...
for file in range(int(sys.argv[1])):  
  ...
  max_time = max(max_time, float(data[data.find("Search time: ")+13 : data.find("s", data.find("Search time: ")+13)]))
  sum_time = sum_time + float(data[data.find("Search time: ")+13 : data.find("s", data.find("Search time: ")+13)])
